...
# Wraps a Transformer block (process an image (sentence) as a sequence of patches (words))
class Adapter(nn.Module):
    def __init__(self, blk) -> None:
        ...
        self.prompt_learn = nn.Sequential(
            # Using 32 as the bottleneck dimension
            nn.Linear(dim, 32),
            nn.GELU(),
            nn.Linear(32, dim),
            nn.GELU()
...
class RFB_modified(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        # 1×1 conv → no receptive field expansion, acts like a baseline
        self.branch0 = nn.Sequential(
            BasicConv2d(in_channel, out_channel, 1),
        )
        # Dilated 3×3 conv → captures medium-scale features
        ...
        # Uses larger asymmetric kernels (1×5 and 5×1) → captures larger context
        ...
        # Dilated 7×7 conv → captures long-range spatial patterns
...
class CATS2d(nn.Module):
    def __init__(self, checkpoint_path=None, model_cfg='sam2_hiera_l.yaml', device='cpu'):
        ...
        self.up3 = (Up(128, 64))
        self.up4 = (Up(128, 64))
        self.side1 = nn.Conv2d(64, 5, kernel_size=1) #changed from 1 to 5
        self.side2 = nn.Conv2d(64, 5, kernel_size=1) #changed from 1 to 5
        self.head = nn.Conv2d(64, 5, kernel_size=1) #changed from 1 to 5
